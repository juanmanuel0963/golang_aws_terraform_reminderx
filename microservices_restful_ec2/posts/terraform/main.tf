...
resource "aws_cloudwatch_event_target" "the_target" {
  ...
  //input     = "{\"commands\":[\"ls -a\"]}"
  //input     = "{\"commands\":[\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins/microservices_restful_ec2/posts/source_code\",\"export PORT=${var.posts_port}\",\"export db_conn='${local.db_conn}'\",\"sudo chmod 700 ./migrate/migrate\",\"sudo --preserve-env ./migrate/migrate\",\"sudo chmod 700 main\",\"sudo --preserve-env ./main\"]}"
  input     = "{\"commands\":[\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins/microservices_restful_ec2/posts/source_code\",\"export PORT=${var.posts_port}\",\"export db_conn='${local.db_conn}'\",\"sudo chmod 700 main\",\"sudo --preserve-env ./main\"]}"
