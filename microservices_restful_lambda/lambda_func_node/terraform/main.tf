...
data "archive_file" "the_lambda_function" {
  ...
  output_path = "${path.cwd}/microservices_restful_lambda/${var.lambda_func_name}/last_deploy/${var.lambda_func_name}.zip"
}
...
//It also sets the runtime to NodeJS 12.x, and assigns the handler to the handler function defined in lambda_func_node.js. 
