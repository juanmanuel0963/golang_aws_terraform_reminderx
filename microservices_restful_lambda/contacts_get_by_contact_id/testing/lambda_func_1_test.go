...
// CloudWatch Logs
// https://us-east-1.console.aws.amazon.com/lambda/home?region=us-east-1#/functions/contacts_get_by_contact_id_romantic_shark?tab=monitoring
...
func Test_with_aws_signature(t *testing.T) {
	...
	//-----------------------------------------------------------
	fmt.Println("Step - Validating if the response len(responseBody.ContactsList) == 1")
	//-----------------------------------------------------------
	//
	if expect, got := true, len(responseBody.ContactsList) == 1; expect != got {
		t.Errorf("Step - Validating if the response len(responseBody.ContactsList) == 1, expect %v, got %v", expect, got)
	}
}
