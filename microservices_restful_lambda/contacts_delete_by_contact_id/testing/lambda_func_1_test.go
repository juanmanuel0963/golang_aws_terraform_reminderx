...
func Test_with_aws_signature(t *testing.T) {
	...
	fmt.Println("Step - Decoding the http response body - Status Code:")
	fmt.Println(responseBody.StatusCode)

	fmt.Println("Step - Decoding the http response body - Contact:")
	fmt.Println(responseBody.Contact)

	//-----------------------------------------------------------
	fmt.Println("Step - Validating if the responseBody.StatusMessage == RECORD_OK || RECORD_NOT_FOUND")
	//-----------------------------------------------------------
	//
	if expect, got := true, responseBody.StatusMessage == "RECORD_OK" || responseBody.StatusMessage == "RECORD_NOT_FOUND"; expect != got {
		t.Errorf("Step - Validating if the responseBody.StatusMessage == RECORD_OK || RECORD_NOT_FOUND, expect %v, got %v", expect, got)
	}
