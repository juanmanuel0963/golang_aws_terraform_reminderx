...
func FunctionHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	//
	log.Printf("Step - Unmarshalling request body - Hello there Id: %v. FirstName: %v. LastName: %v.", requestContact.Id, requestContact.FirstName, requestContact.LastName)
	//
	//---------------------------------------------------------------------------------
	//----------------------------DB Connection Test-----------------------------------
	//
	log.Println("Step - DB connection string formatting")
	//
	...
	errQuery := db.QueryRow("SELECT id FROM contacts_delete_by_contact_id($1)", requestContact.Id).Scan(&responseContact.Id)
	...
	if errQuery != nil {
		messageStatus := "RECORD_NOT_FOUND"
	...
	//
	//switch contact.Id = deletedContactId; { // missing expression means "true"
