...
func FunctionHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	rows, errQuery := db.Query("SELECT id, first_name, last_name, email, created_at, company_id FROM contacts_get_by_dynamic_filter($1, $2, $3, $4)", requestContactFilter.Id, requestContactFilter.SearchText, requestContactFilter.CreatedAtStart, requestContactFilter.CreatedAtFinish)
