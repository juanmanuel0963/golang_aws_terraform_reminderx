...
func FunctionHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	errQuery := db.QueryRow("SELECT id FROM contacts_update_by_contact_id($1, $2, $3, $4, $5)", requestContact.Id, requestContact.CompanyId, requestContact.FirstName, requestContact.LastName, requestContact.Email).Scan(&responseContact.Id)
	...
	//switch responseContact.Id = updatedContactId; { // missing expression means "true"
