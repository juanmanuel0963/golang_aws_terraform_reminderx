...
resource "aws_cloudwatch_event_target" "the_target" {
  ...
  //input     = "{\"commands\":[\"ls -a\"]}"
  input     = "{\"commands\":[\"export HOME=/home/ubuntu\",\"export GOPATH=$HOME/go\",\"export GOMODCACHE=$HOME/go/pkg/mod\",\"export GOCACHE=$HOME/.cache/go-build\",\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins\",\"cd microservices_grpc_ec2/usermgmt_op2_in_memory/usermgmt_server\",\"sudo chmod 700 usermgmt_server\",\"sudo --preserve-env ./usermgmt_server\"]}"
  //input     = "{\"commands\":[\"export HOME=/home/ubuntu\",\"export GOPATH=$HOME/go\",\"export GOMODCACHE=$HOME/go/pkg/mod\",\"export GOCACHE=$HOME/.cache/go-build\",\"cd /home/ubuntu/\",\"cd golang_aws_terraform_jenkins\",\"cd microservices_grpc_ec2/usermgmt_op2_in_memory/usermgmt_server\",\"go run usermgmt_server.go\"]}"
